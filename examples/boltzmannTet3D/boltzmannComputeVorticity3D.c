...
void boltzmannComputeVorticity3D(mesh3D *mesh, dfloat *q, iint outfld, iint Nfields){
  ...
  for(iint e=0;e<mesh->Nelements;++e){
    ...
    for(iint n=0;n<mesh->Np;++n){
      ...
      dfloat dudy = mesh->vgeo[mesh->Nvgeo*e+RYID]*dudr + mesh->vgeo[mesh->Nvgeo*e+SYID]*duds;
      dfloat dvdx = mesh->vgeo[mesh->Nvgeo*e+RXID]*dvdr + mesh->vgeo[mesh->Nvgeo*e+SXID]*dvds;
