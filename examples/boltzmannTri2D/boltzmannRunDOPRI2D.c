...
void boltzmannRunDOPRI2D(bns_t *bns, int haloBytes, dfloat * sendBuffer, dfloat *recvBuffer, char * options){
 ...
 //  dfloat dtMIN = 1E-10; //minumum allowed timestep
    ...
    // Perform an Full RK Step
		if(strstr(options,"XDOPRI"))
			boltzmannXDOPRIStep2D(bns,time, haloBytes,sendBuffer, recvBuffer, options);
