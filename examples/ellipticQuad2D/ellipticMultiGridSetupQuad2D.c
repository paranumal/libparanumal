...
dfloat *buildCoarsenerQuad2D(mesh2D** meshLevels, int N, int Nc) {
  ...
  *vlambda = lambda;
  ...
  for (int n=0;n<numLevels;n++) {
    ...
    if (n==0) {
      ...
      solverL = ellipticBuildMultigridLevelQuad2D(solver,levelDegree,n,options);
    }
    ...
    R[n] = buildCoarsenerQuad2D(meshLevels, N, Nc);
