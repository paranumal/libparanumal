...
void loadElementStiffnessMatricesTri2D(mesh2D *mesh, const char *options, int N){
  ...
  iint *IndT;  
  SrrT = (dfloat *) calloc(mesh->maxNnzPerRow*mesh->Np,sizeof(dfloat));
  SrsT = (dfloat *) calloc(mesh->maxNnzPerRow*mesh->Np,sizeof(dfloat));
  SssT = (dfloat *) calloc(mesh->maxNnzPerRow*mesh->Np,sizeof(dfloat));
  IndT = (iint*)   calloc(mesh->maxNnzPerRow*mesh->Np,sizeof(iint));
  printf("\nIND transose\n");
  for (iint n=0;n<mesh->Np;n++) {
    for (iint m=0;m<mesh->maxNnzPerRow;m++) {  
      ...
      printf("IndT[%d] =  %d \n",m+n*mesh->maxNnzPerRow,  IndT[m+n*mesh->maxNnzPerRow]);   
    }
    printf("\n");
  }
  ...
  /*char4 packing*/
  int paddedRowSize = maxNnzPerRow+ (4-(maxNnzPerRow%4));
