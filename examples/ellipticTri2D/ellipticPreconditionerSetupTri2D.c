...
void dampedJacobiTri2D(void **args, occa::memory o_r, occa::memory o_x, bool x_is_zero) {
  ...
  solver->dotMultiplyKernel(mesh->Np*mesh->Nelements,*o_invDiagA,o_r,o_r);  
...
precon_t *ellipticPreconditionerSetupTri2D(solver_t *solver, ogs_t *ogs, dfloat tau, dfloat lambda, iint *BCType, const char *options){
  ...
  if(strstr(options, "OAS")||strstr(options, "OMS")){
    ...
    //maually build nultigrid levels
    ...
    dfloat *invDiagA;
    ellipticBuildJacobiIpdgTri2D(mesh, mesh->Np, NULL,
                                   tau, lambda, BCType, &invDiagA,options);
