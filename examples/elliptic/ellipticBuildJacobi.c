...
void BuildLocalBRdgPatchAxTri2D(elliptic_t* elliptic, mesh2D* mesh, int basisNp, dfloat *basis, dfloat tau, dfloat lambda, int* BCType,
                        dfloat *MS, dlong eM, dfloat *A);
...
void ellipticBuildJacobiTri2D(elliptic_t* elliptic, mesh2D* mesh, int basisNp, dfloat *basis,
  ...
  if (strstr((options.getArgs("DISCRETIZATION")).c_str(),"CONTINUOUS")) {
  ...
  #pragma omp parallel
  {
    ...
    for(dlong eM=0;eM<mesh->Nelements;++eM){
      ...
      } else if (strstr((options.getArgs("DISCRETIZATION")).c_str(),"CONTINUOUS")) {
