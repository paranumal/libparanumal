...
int pcg(elliptic_t* elliptic, setupAide options, dfloat lambda, 
  ...
  dfloat normB = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_r, o_r, options);
  dfloat TOL =  mymax(tol*tol*normB,tol*tol);
  // compute A*x
  ellipticOperator(elliptic, lambda, o_x, elliptic->o_Ax, options);

  // subtract r = b - A*x
  ellipticScaledAdd(elliptic, -1.f, o_Ax, 1.f, o_r);

  dfloat rdotr0 = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_r, o_r, options);
  ...
  // Precon^{-1} (b-A*x)
  ellipticPreconditioner(elliptic, lambda, o_r, o_z, options);

  // p = z
  o_p.copyFrom(o_z); // PCG

  // dot(r,z)
  rdotz0 = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_r, o_z, options);
  dfloat rdotr1 = 0;
  dfloat rdotz1 = 0;
  ...
  while((Niter <MAXIT)) {
    ...
    // A*p
    ellipticOperator(elliptic, lambda, o_p, o_Ap, options);

    // dot(p,A*p)
    pAp =  ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree,o_p, o_Ap, options);
    ...
    // dot(r,r)
    rdotr1 = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_r, o_r, options);
    ...
    // dot(r,z)
    rdotz1 = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_r, o_z, options);

    // flexible pcg beta = (z.(-alpha*Ap))/zdotz0
    if(options.compareArgs("KRYLOV SOLVER", "FLEXIBLE PCG")) {
      dfloat zdotAp = ellipticWeightedInnerProduct(elliptic, elliptic->o_invDegree, o_z, o_Ap, options);
