...
void acousticsPmlSurface3Dbbdg(mesh3D *mesh, int lev, dfloat time){
  ...
  dfloat *fluxpz = (dfloat*) calloc(mesh->NfpMax*mesh->Nfaces,sizeof(dfloat));
  ...
  for(int et=0;et<mesh->MRABpmlNelements[lev];++et){
    ...
    for(int n=0;n<mesh->Nfp[N]*mesh->Nfaces;++n){
      ...
      for (int m = 0; m < 7; ++m){
        ...
        pxtmpflux += L0val * fluxpx[L0id+f*mesh->Nfp[N]];
        pytmpflux += L0val * fluxpy[L0id+f*mesh->Nfp[N]];
        pztmpflux += L0val * fluxpz[L0id+f*mesh->Nfp[N]];
      }
