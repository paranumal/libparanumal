...
int main(int argc, char **argv){
  ...
  dfloat *q    = (dfloat*) calloc(mesh->Nelements*mesh->Np, sizeof(dfloat));
  dfloat *dqdx = (dfloat*) calloc(mesh->Nelements*mesh->Np, sizeof(dfloat));
  dfloat *dqdy = (dfloat*) calloc(mesh->Nelements*mesh->Np, sizeof(dfloat));
  ...
  // run a test kernel
  //occaTestQuad2D(mesh, q, dqdx, dqdy);

  // run timings for 9 versions of meshGradient kernel 
  occaOptimizeGradientQuad2D(mesh, q, dqdx, dqdy);
