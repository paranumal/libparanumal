...
void acousticsPmlSurface2Dbbdg(mesh2D *mesh, iint lev, dfloat t){
  ...
  for(iint et=0;et<mesh->MRABpmlNelements[lev];++et){
    ...
    for(iint n=0;n<mesh->Nfp*mesh->Nfaces;++n){
      ...
      // compute (q^* - q^-)
      dfloat dpxS = 0.5f*(uP-uM) + mesh->Lambda2*(-nx*(uP-uM));
      dfloat dpyS = 0.5f*(vP-vM) + mesh->Lambda2*(-ny*(vP-vM));

      // evaluate "flux" terms: (sJ/J)*(A*nx+B*ny)*(q^* - q^-)
      fluxpx[n] = invJ*sJ*(-nx*dpxS);
      fluxpy[n] = invJ*sJ*(-ny*dpyS);
    }
    ...
    for(iint n=0;n<mesh->Np;++n){
      iint id = 3*mesh->pmlNfields*(mesh->Np*pmlId + n) + mesh->pmlNfields*mesh->MRABshiftIndex[lev];

      // load RHS
      dfloat rhsqnpx = mesh->pmlrhsq[id+0];
      dfloat rhsqnpy = mesh->pmlrhsq[id+1];
      ...
      mesh->pmlrhsq[id+0] = rhsqnpx;
      mesh->pmlrhsq[id+1] = rhsqnpy;
    }
  }

  // free temporary flux storage
  free(fluxpx); free(fluxpy);
  free(fluxpx_copy); free(fluxpy_copy); 
}
