...
void acousticsPml3D(mesh3D *mesh, int lev){
  ...
  dfloat *cubrhsw = (dfloat *) calloc(mesh->cubNp,sizeof(dfloat));
  ...
  dfloat *cubrhswtilde = (dfloat *) calloc(mesh->cubNp,sizeof(dfloat));
  ...
  for(iint m=0;m<mesh->MRABpmlNelements[lev];++m){
    ...
    for(iint n=0;n<mesh->cubNp;++n){
      ...
      for (iint i=0;i<mesh->Np;++i){
        ...
        p += mesh->cubInterp[n*mesh->Np + i] * mesh->q[base+3];
        ...
        wtilde += mesh->cubInterp[n*mesh->Np + i] * mesh->pmlq[pmlBase+2];
        ptilde += mesh->cubInterp[n*mesh->Np + i] * mesh->pmlq[pmlBase+3];
      }    
      ...
      cubrhsw[n] = -(sigmay-sigmax)*v - sigmax*(sigmax-sigmay)*vtilde; // vhat
...
// function to compute collocation differentiation
// contributions to nodal DG rhs for acoustics
void acousticsPml3D_wadg(mesh3D *mesh, int lev){
