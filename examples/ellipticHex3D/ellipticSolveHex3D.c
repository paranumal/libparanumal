...
int ellipticSolveHex3D(solver_t *solver, dfloat lambda, dfloat tol,
...
/*
  ...
  for (int n=0;n<mesh->Np*mesh->Nelements;n++) {
    ...
    //ellipticParallelGatherScatterHex3D(mesh, mesh->ogs, o_t, dfloatString, "add");
    ...
    //solver->dotMultiplyKernel(mesh->Nelements*mesh->Np, mesh->ogs->o_invDegree, o_Pt, o_Pt);
    //ellipticParallelGatherScatterTri3D(mesh, mesh->ogs, o_Pt, dfloatString, "add");
...
*/
  ...
  if(strstr(options,"VERBOSE")){
    ...
    if (rank==0){
      printf("%03d %03d "hlongFormat" "hlongFormat" %d %17.15lg %3.5g \t [ RANKS N NELEMENTS DOFS ITERATIONS ELAPSEDTIME PRECONMEMORY] \n",
