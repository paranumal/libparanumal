...
// currently maximum
void insErrorHex3D(ins_t *ins, dfloat time,char *options){
  ...
  if(rank==0)
    printf("Time: %g minU: %g maxU: %g minV: %g maxV: %g minW: %g maxW: %g minP: %g maxP: %g\n", 
             time, gMinU, gMaxU, gMinV, gMaxV, gMinW, gMaxW, gMinP, gMaxP );
