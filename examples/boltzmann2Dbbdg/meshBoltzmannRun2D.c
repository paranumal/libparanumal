...
void meshBoltzmannRun2D(mesh2D *mesh){
  ...
  for(iint tstep=0;tstep<mesh->NtimeSteps;++tstep){
	...
	memcpy(mesh->q+mesh->Np*mesh->Nfields*mesh->Nelements, recvBuffer, haloBytes);
      ...
      meshBoltzmannSurface2D(mesh, t);
      
      // update solution using Euler Forward
      meshBoltzmannUpdate2D(mesh, mesh->rka[rk], mesh->rkb[rk]);
...
void meshBoltzmannOccaRun2D(mesh2D *mesh){
  ...
  for(iint tstep=0;tstep<mesh->NtimeSteps;++tstep){
	...
	meshHaloExchangeStart2D(mesh,
      ...
      // compute volume contribution to DG boltzmann RHS
      mesh->boltzmannVolumeKernel(mesh->Nelements,
				  mesh->o_vgeo,
				  mesh->o_DrT,
				  mesh->o_DsT,
				  mesh->o_q,
	...
	mesh->o_q.copyFrom(recvBuffer, haloBytes, offset);
      ...
      // compute surface contribution to DG boltzmann RHS
      mesh->boltzmannSurfaceKernel(mesh->Nelements,
				   mesh->o_sgeo,
				   mesh->o_LIFTT,
				   mesh->o_vmapM,
				   mesh->o_vmapP,
      ...
      mesh->boltzmannUpdateKernel(mesh->Nelements*mesh->Np*mesh->Nfields,
				  mesh->dt,
				  mesh->rka[rk],
				  mesh->rkb[rk],
				  mesh->o_rhsq,
...
void meshBoltzmannPmlOccaRun2D(mesh2D *mesh){
  ...
  for(iint tstep=0;tstep<mesh->NtimeSteps;++tstep){
	...
	meshHaloExchangeStart2D(mesh,
      ...
      // compute volume contribution to DG boltzmann RHS
      mesh->boltzmannPmlVolumeKernel(mesh->Nelements,
				     mesh->o_vgeo,
				     mesh->o_sigmax,
				     mesh->o_DrT,
				     mesh->o_DsT,
	...
	mesh->o_q.copyFrom(recvBuffer, haloBytes, offset);
      ...
      // compute surface contribution to DG boltzmann RHS
      mesh->boltzmannPmlSurfaceKernel(mesh->Nelements,
				      mesh->o_sgeo,
				      mesh->o_LIFTT,
				      mesh->o_vmapM,
				      mesh->o_vmapP,
...
void meshBoltzmannOccaAsyncRun2D(mesh2D *mesh){
...
void meshBoltzmannSplitSurfaceOccaAsyncRun2D(mesh2D *mesh){
  ...
  for(iint tstep=0;tstep<mesh->NtimeSteps;++tstep){
	...
	// switch back to main stream
      ...
      // compute surface contribution to DG boltzmann RHS
      mesh->boltzmannPartialSurfaceKernel(mesh->NinternalElements,
	...
	// compute surface contribution to DG boltzmann RHS on stream 0
	mesh->boltzmannPartialSurfaceKernel(mesh->NnotInternalElements,
