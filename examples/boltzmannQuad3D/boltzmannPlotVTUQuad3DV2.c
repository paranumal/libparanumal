...
void boltzmannPlotVTUQuad3DV2(mesh_t *mesh, char *fileNameBase, int tstep){
  ...
  for(iint e=0;e<mesh->Nelements;++e){
	...
	dfloat dudx = rx*dudr + sx*duds + tx*mesh->q[base + 1*mesh->Np];
	dfloat dudy = ry*dudr + sy*duds + ty*mesh->q[base + 1*mesh->Np];
	dfloat dudz = rz*dudr + sz*duds + tz*mesh->q[base + 1*mesh->Np];

	dfloat dvdx = rx*dvdr + sx*dvds + tx*mesh->q[base + 2*mesh->Np];
	dfloat dvdy = ry*dvdr + sy*dvds + ty*mesh->q[base + 2*mesh->Np];
	dfloat dvdz = rz*dvdr + sz*dvds + tz*mesh->q[base + 2*mesh->Np];

	dfloat dwdx = rx*dwdr + sx*dwds + tx*mesh->q[base + 3*mesh->Np];
	dfloat dwdy = ry*dwdr + sy*dwds + ty*mesh->q[base + 3*mesh->Np];
	dfloat dwdz = rz*dwdr + sz*dwds + tz*mesh->q[base + 3*mesh->Np];
