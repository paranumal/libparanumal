...
void ellipticOperator3D(solver_t *solver, dfloat lambda,
  ...
  if(strstr(options, "CONTINUOUS")){
...
#if 0
    solver->AxKernel(mesh->Nelements, solver->o_gjGeo, solver->o_gjD, solver->o_gjD2, solver->o_gjI, lambda, o_q, o_Aq);
	...
	solver->partialAxKernel(solver->NglobalGatherElements, solver->o_globalGatherElementList,
				solver->o_gjGeo, solver->o_gjD, solver->o_gjD2, solver->o_gjI, lambda, o_q, o_Aq, 
				solver->o_grad);
      }
    ...
    for (int m=0;m<solver->NlocalGatherElements;m++) {
      int e = solver->localGatherElementList[m];
    ...
    for (int m=0;m<solver->NglobalGatherElements;m++) {
      int e = solver->globalGatherElementList[m];
      for (int n=0;n<mesh->Np;n++) {
