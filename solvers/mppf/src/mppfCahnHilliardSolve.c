...
void mppfCahnHilliardSolve(mppf_t *mppf, dfloat time){
  ...
  if (mppf->phiOptions.compareArgs("DISCRETIZATION","CONTINUOUS")) {
    ...
    if (psiSolver->Nmasked) 
      mesh->maskKernel(psiSolver->Nmasked, psiSolver->o_maskIds, mppf->o_rhsPhi);
    ...
    occaTimerTic(mesh->device,"CahnHilliardRhsBC");    
    mppf->phaseFieldRhsIpdgBCKernel(mesh->Nelements,
                                  ...
                                  mesh->o_MM,
                                  mppf->o_rhsPhi);
    occaTimerToc(mesh->device,"CahnHilliardRhsBC");   
  }

  //copy current field as initial guess? (could use Uhat or beter guess)
  // dlong Ntotal = (mesh->Nelements+mesh->totalHaloPairs)*mesh->Np;
  // mppf->o_rkPhi.copyFrom(mppf->o_Psi,Ntotal*sizeof(dfloat),0, 0*mppf->fieldOffset*sizeof(dfloat));

  if (mppf->phiOptions.compareArgs("DISCRETIZATION","CONTINUOUS")) {
    if (psiSolver->Nmasked){ 
      mesh->maskKernel(psiSolver->Nmasked, psiSolver->o_maskIds, mppf->o_Psi);
    }
  }
...
if (mppf->phiOptions.compareArgs("DISCRETIZATION","CONTINUOUS")) {
 ...
 if (mppf->phiOptions.compareArgs("DISCRETIZATION","CONTINUOUS")) {
    ...
    ogsGatherScatter(mppf->o_rhsPhi, ogsDfloat, ogsAdd, mesh->ogs);
    if (phiSolver->Nmasked) 
      mesh->maskKernel(phiSolver->Nmasked, phiSolver->o_maskIds, mppf->o_rhsPhi);
  } else if (mppf->phiOptions.compareArgs("DISCRETIZATION","IPDG")) {
    // Currently we do not need that deuto homegenous bcs need to be used for more complex bcs
  ...
  if (mppf->phiOptions.compareArgs("DISCRETIZATION","CONTINUOUS")) {
    if (phiSolver->Nmasked){ 
      mesh->maskKernel(phiSolver->Nmasked, phiSolver->o_maskIds, mppf->o_rkPhi);
    }
  }
