...
#define p_Nwarps ((int)(p_Nq2+32-1)/32)
...
__device__ void advectionMassMatrixMultiply(const dlong element, const dfloat * __restrict__  r_WJ, dfloat s_Ap[p_cubNq][p_cubNq][p_cubNq]){
...
#pragma unroll 8
    ...
    advectionMassMatrixMultiply(element, r_WJ, s_tmp1);
...
int main(int argc, char **argv){
  ...
  for(int test=0;test<Ntests;++test)
    advectionInvertMassMatrix <<< G, B >>>
      (Nelements, c_elementIds, c_cubvgeo, c_I, c_precon, c_rhsq, c_q);
