...
*/
{
  ...
  if ( sendtype == MPI_BYTE )
    {
      ierror = mpi_copy_byte( (char*)(sendbuf), (char*) recvbuf, sendcount );
    }
    ...
    {
      ierror = mpi_copy_float ( (float*) sendbuf,(float*) recvbuf, sendcount );
    }
...
*/
{
  ...
  if ( datatype == MPI_DOUBLE )
    {
      ierror = mpi_reduce_double ( (double*)data1, (double*)data2, n, operation );
    }
  else if ( datatype == MPI_FLOAT )
    {
      ierror = mpi_reduce_float ( (float*)data1, (float*)data2, n, operation );
    }
  else if ( datatype == MPI_INT )
    {
      ierror = mpi_reduce_int ( (int*)data1, (int*)data2, n, operation );
    }
  else
    {
      ierror = MPI_FAILURE;
...
*/
{
  ...
  if ( datatype == MPI_BYTE )
    {
      ierror = mpi_copy_byte ( (char*) data1, (char*)data2, n );
    }
  else if ( datatype == MPI_DOUBLE )
    {
      ierror = mpi_copy_double ( (double*)data1, (double*)data2, n );
    }
  else if ( datatype == MPI_FLOAT )
    {
      ierror = mpi_copy_float ( (float*) data1, (float*)data2, n );
    }
  else if ( datatype == MPI_INT )
    {
      ierror = mpi_copy_int ( (int*)data1, (int*)data2, n );
    }
  else
    {
      ierror = MPI_FAILURE;
...
int MPI_Waitall ( int icount, MPI_Request *request, MPI_Status *status )
