...
void meshParallelConnectNodesQuad2D(mesh2D *mesh){
  ...
  while(globalChange>0){
    ...
    meshHaloExchange2D(mesh, mesh->Np*sizeof(parallelNode_t), globalNumbering, sendBuffer, globalNumbering+mesh->Np*mesh->Nelements);
  ...
  for(iint e=0;e<mesh->Nelements;++e){
	...
	printf("Node: (e,n,r,id,bc) = (%d,%d,%d,%d,%d) => (%d,%d,[%d],%d,(%g,%g),%d) => (%d,%d,[%d],%d,(%g,%g),%d)\n", 
	       e,
	       n,
	       rank,
	       id,
