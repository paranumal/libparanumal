...
void meshExtendGridQuad3D(mesh_t *mesh) {
  ...
  mesh->eAdjacent = (iint *) calloc((mesh->NgridElements - mesh->Nelements),sizeof(iint));
  iint count_extra = 0;
...
void meshPreserveGridQuad3D(mesh_t *mesh) {
  mesh->NgridElements = mesh->Nelements;
  mesh->gridToE = mesh->EToE;
}
